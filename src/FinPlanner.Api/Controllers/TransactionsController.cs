...
public sealed class TransactionsController(FinPlannerDbContext dbContext, CurrentUserContext currentUser) : TenantControllerBase(currentUser)
{
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<TransactionResponse>>> List([FromQuery] DateOnly? from, [FromQuery] DateOnly? to, [FromQuery] string? type, CancellationToken cancellationToken)
    {
        ...
        var query = dbContext.ActualTransactions.AsNoTracking().Where(item => item.FamilyId == familyId);
    ...
    {
        if (RequireFamily(out var familyId) is { } unauthorized) return unauthorized;
        var item = await dbContext.ActualTransactions.AsNoTracking().SingleOrDefaultAsync(transaction => transaction.Id == id && transaction.FamilyId == familyId, cancellationToken);
