...
public sealed class PlanningController(FinPlannerDbContext dbContext, CurrentUserContext currentUser) : TenantControllerBase(currentUser)
{
    ...
    public async Task<ActionResult<PlannedTransactionResponse>> CreatePlanned(CreatePlannedTransactionRequest request, CancellationToken cancellationToken)
    {
        ...
        var item = new Domain.PlannedTransaction { FamilyId = familyId, PlannedDate = request.Date, Amount = request.Amount, FromAccountId = request.FromAccountId, ToAccountId = request.ToAccountId, CategoryId = request.CategoryId, RecurringRuleId = request.RecurringRuleId, Description = request.Description, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow };
    ...
    {
        if (RequireFamily(out var familyId) is { } unauthorized) return unauthorized;
        var query = dbContext.PlannedTransactions.AsNoTracking().Where(item => item.FamilyId == familyId);
    ...
    public async Task<ActionResult<IReadOnlyList<RecurringRuleResponse>>> RecurringRules(CancellationToken cancellationToken)
    {
        if (RequireFamily(out var familyId) is { } unauthorized) return unauthorized;
        return Ok(await dbContext.RecurringRules.AsNoTracking().Where(item => item.FamilyId == familyId).OrderBy(item => item.Name).Select(item => new RecurringRuleResponse(item.Id, item.Name, item.Amount, item.Frequency, item.StartDate, item.EndDate, item.Active, item.Description)).ToListAsync(cancellationToken));
    }
    ...
    public async Task<ActionResult<RecurringRuleResponse>> CreateRecurring(CreateRecurringRuleRequest request, CancellationToken cancellationToken)
    {
        ...
        var validation = await Validate(familyId, request.Amount, request.FromAccountId, request.ToAccountId, request.CategoryId, cancellationToken);
        if (validation is not null) return validation;
        var item = new Domain.RecurringRule { FamilyId = familyId, Name = request.Name.Trim(), Amount = request.Amount, FromAccountId = request.FromAccountId, ToAccountId = request.ToAccountId, CategoryId = request.CategoryId, Frequency = request.Frequency, DayOfMonth = request.DayOfMonth, StartDate = request.StartDate, EndDate = request.EndDate, Description = request.Description, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow };
