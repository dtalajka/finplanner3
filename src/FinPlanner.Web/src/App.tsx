...
function App() {
  ...
  return <div className={`app-shell ${activeView.toLowerCase()}`}>
    ...
    <main className="main-content"><header className="topbar"><div className="breadcrumb">Workspace <span>/</span> {activeView}</div><div className="top-actions">{activeView === 'Accounts' ? <button className="add-button" onClick={openAccountCreate} type="button"><span>+</span> Add account</button> : accounts.length ? <button className="add-button" onClick={openCreate} type="button"><span>+</span> Add transaction</button> : <button className="add-button" onClick={openAccountCreate} type="button"><span>+</span> Create account</button>}</div></header>
      <div className="content-wrap"><section className="page-heading"><div><p className="eyebrow">Household activity</p><h1>{activeView === 'Accounts' ? 'Accounts' : 'Transactions'}</h1><p className="muted">{activeView === 'Accounts' ? 'Manage the accounts used by your household.' : 'Track every income and expense across your family accounts.'}</p></div>{activeView !== 'Accounts' && <div className="period-controls"><button className="month-arrow" aria-label="Previous month" onClick={() => setPeriod(current => shiftPeriod(current, -1))} type="button">‹</button><input aria-label="Month" type="month" value={period} onChange={event => setPeriod(event.target.value)} /><button className="month-arrow" aria-label="Next month" onClick={() => setPeriod(current => shiftPeriod(current, 1))} type="button">›</button><select aria-label="Transaction type" value={typeFilter} onChange={event => setTypeFilter(event.target.value)}><option value="">All types</option><option value="Income">Income</option><option value="Expense">Expenses</option></select></div>}</section>
