...
function App() {
  ...
  useEffect(() => { void loadData() }, [period, typeFilter])
  ...
  const createAccount = async (event: FormEvent) => {
    ...
    try {
      await request<Account>('/accounts', { method: 'POST', body: JSON.stringify({ name: accountName, type: 'Checking', currency: 'EUR' }) })
      setAccountName(''); setShowAccountComposer(false); await loadData()
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><div className="breadcrumb">Workspace <span>/</span> {activeView}</div><div className="top-actions">{accounts.length ? <button className="add-button" onClick={openCreate} type="button"><span>+</span> Add transaction</button> : <button className="add-button" onClick={() => setShowAccountComposer(true)} type="button"><span>+</span> Create account</button>}</div></header>
      <div className="content-wrap"><section className="page-heading"><div><p className="eyebrow">Household activity</p><h1>Transactions</h1><p className="muted">Track every income and expense across your family accounts.</p></div><button className="period-button" type="button">{period} <span>⌄</span></button></section>
    ...
    {showAccountComposer && <div className="modal-backdrop" role="presentation" onClick={() => !saving && setShowAccountComposer(false)}><form className="modal" onSubmit={createAccount} onClick={event => event.stopPropagation()}><div className="modal-heading"><div><p className="eyebrow">Setup</p><h2>Create account</h2></div><button className="close-button" aria-label="Close" onClick={() => setShowAccountComposer(false)} type="button">×</button></div><label>Account name<input required maxLength={120} value={accountName} onChange={event => setAccountName(event.target.value)} placeholder="e.g. Main checking" /></label><p className="muted">A checking account in EUR will be created for your household.</p><button className="add-button full" disabled={saving} type="submit">{saving ? 'Creating...' : 'Create account'}</button></form></div>}
  </div>
}
